...
def generate_image_with_dalle(prompt: List[Dict[str, Any]], number_of_images: int = 1) -> Tuple[Optional[List[Image.Image]], Optional[str]]:
    """
    DALL-E 3 APIを使用して画像を生成する関数
    ...
    if not OPENAI_API_KEY:
        return None, "OpenAI APIキーが設定されていません。環境変数OPENAI_API_KEYを設定してください。"
    
    try:
        # プロンプトを最適化（OpenAI用）
        optimized_prompt, error = generate_imagen_prompt(prompt, provider="openai")
        if error:
        ...
        image_model = provider_info.get("image_model", "dall-e-3")
        ...
        response = client.images.generate(
            ...
            n=number_of_images,
            size="1024x1024",  # DALL-E 3 のデフォルトサイズ
            quality="standard",
        ...
        error_msg = f"DALL-E APIによる画像生成中にエラーが発生しました: {str(e)}"
        print(error_msg)
...
def should_generate_image(prompt: str) -> bool:
    ...
    """
    if not OPENAI_API_KEY:
        print("OpenAI APIキーが設定されていないため、画像生成判定ができません")
        return False
...
def call_openai_api(
    ...
    戻り値:
        generate_image=False の場合: OpenAI APIからの応答テキスト
...
"""
    ...
    try:
        # 画像生成モードが有効な場合
        if generate_image:
            print("DALL-E 3 APIを使用して画像を生成します")
            
            # DALL-E 3 APIを使用して画像を生成（1枚のみ）
            images, error = generate_image_with_dalle(messages, number_of_images=1)
        ...
        return f"OpenAI APIへのリクエスト中にエラーが発生しました: {str(e)}"
...
def call_openai_api_streaming(
    ...
    """
    OpenAI APIをストリーミングモードで呼び出して応答を取得する関数
    ...
    """
    if not OPENAI_API_KEY:
        error_msg = "OpenAI APIキーが設定されていません。環境変数OPENAI_API_KEYを設定してください。"
        if callback:
...
"""
    ...
    try:
        ...
        error_msg = f"OpenAI APIへのリクエスト中にエラーが発生しました: {str(e)}"
        if callback:
            callback(error_msg, True)
        yield error_msg
